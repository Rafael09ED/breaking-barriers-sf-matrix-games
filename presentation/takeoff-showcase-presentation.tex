...
\begin{frame}[plain]
  ...
  \vspace{0.7em}
  {\Large\bfseries Argue. \textcolor{Signal}{Adjudicate}. Commit.}
\end{frame}
\note{
...
% -----------------------------------------------------------------------------
\begin{frame}{The missing interface to AI 2027}
  \begin{columns}[c,onlytextwidth]
    ...
    \begin{column}{0.38\textwidth}
      \terminalbox{%
        ...
        \mono{online AGI wargame}\\
        \mono{LLM-driven characters}\\[0.35em]
        {\color{Signal}\mono{millions of players}}
      }
    \end{column}
  \end{columns}
...
\end{frame}
\note{
  The AI Futures Project has run its in-person exercise about 35 times, including
  participants from leading labs and congressional staff. In 2025 its COO publicly
  asked for an online version with LLM-driven characters. TAKEOFF is an unaffiliated
  prototype response: a facilitator-light way to explore the scenario.
}
...
\begin{frame}{One rule keeps the models honest}
  \centering
  \begin{tikzpicture}[
    step/.style={draw=Muted!50,fill=TerminalPanel,minimum width=2.3cm,
      minimum height=1.0cm,align=center,font=\bfseries},
    arrow/.style={-Stealth,thick,draw=Signal}
  ]
    \node[step] (argue) {ARGUE};
    \node[step,right=0.55cm of argue] (judge) {ADJUDICATE};
    \node[step,right=0.55cm of judge] (roll) {ROLL};
    \node[step,right=0.55cm of roll] (commit) {COMMIT};
...
\begin{frame}[plain]
  ...
  \vspace{1.3em}
  \terminalbox{%
    {\color{Signal}\mono{ROADMAP}}\quad{\color{State}\mono{\$ takeoff --seat ALIGN}}\\
    {\color{Muted}\mono{Your move.}}
  }

  \vspace{0.7em}
  {\small\color{Muted}TAKEOFF \textbullet{} Rafael \textbullet{} unofficial AI 2027 project}
\end{frame}
\note{
...
\begin{frame}[noframenumbering]{References \hfill \textcolor{Muted}{BACKUP}}
  ...
  \vfill
  {\color{Signal}\mono{NOVELTY}} No public tool combines Engle-style matrix game
  adjudication, LLM players, and an AI takeoff scenario.
\end{frame}
