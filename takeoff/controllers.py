...
from takeoff.models import ActorId, Argument, PlayerContext, PlayerProposal
from takeoff.openrouter import ModelOutputError, StructuredModelClient
from takeoff.prompts import (
    corrective_human_turn_parser_messages,
    corrective_player_messages,
    human_turn_parser_messages,
    player_messages,
)
...
class RoutingController:
    ...
    def propose(self, context: PlayerContext) -> ProposalResult:
        controller = (
            self._human if context.actor.id == self._human_actor else self._llm
        )
        return controller.propose(context)


class HumanTurnParser:
    def __init__(self, client: StructuredModelClient) -> None:
        self._client = client

    def parse(self, context: PlayerContext, submission: str) -> ProposalResult:
        messages = human_turn_parser_messages(context, submission)
        ...
        for attempt in range(1, 3):
            request_messages = (
                messages
                if attempt == 1
                else corrective_human_turn_parser_messages(messages, last_error)
            )
            try:
                content = self._client.generate(
                    request_messages,
                    PlayerProposal,
                    "takeoff_human_argument",
                )
        ...
        raise ModelOutputError(
            "human submission could not be parsed after 2 attempts: "
            + " | ".join(errors)
        )
